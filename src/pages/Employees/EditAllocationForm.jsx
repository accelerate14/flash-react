...
import {
  TextField, Select, MenuItem, FormControl, Typography, Button
} from '@mui/material';

const EditAllocationForm = ({ onSuccess, onCancel, allocationData }) => {
  const [editAllocationData, setEditAllocationData] = useState({
    ...
    ReportingManagerID: allocationData?.Reporting_Manager_ID || '',
    DepartmentID: allocationData?.Department_ID || '',
    JobPositionID: allocationData?.JobPosition_ID || '',
    LocationID: allocationData?.Location_ID || '',
    ProjectID: allocationData?.Project_ID || '',
    ParticipationID: allocationData?.Participation_ID || '',
    ALLOCATION: allocationData?.ALLOCATION || '',
    COMMENTS: allocationData?.COMMENTS || '',
    C_SOW_Start: allocationData?.C_SOW_Start ? allocationData.C_SOW_Start.substring(0, 10) : '', // Format date
    C_SOW_End: allocationData?.C_SOW_End ? allocationData.C_SOW_End.substring(0, 10) : '',     // Format date
  });
  ...
  useEffect(() => {
    if (allocationData) {
      setEditAllocationData({
        ...
        ALLOCATION: allocationData.ALLOCATION || '',
        COMMENTS: allocationData.COMMENTS || '',
        C_SOW_Start: allocationData.C_SOW_Start ? allocationData.C_SOW_Start.substring(0, 10) : '',
        C_SOW_End: allocationData.C_SOW_End ? allocationData.C_SOW_End.substring(0, 10) : '',
      });
    }
  }, [allocationData]);
  ...
  const handleInputChange = (event) => {
    ...
    setEditAllocationData(prevData => ({
      ...prevData,
      [name]: value,
    }));
  };
  ...
  const handleSaveEditAllocation = async () => {
    if (!editAllocationData.ALLOCATION) {
      alert('Allocation field must not be empty');
      return;
    }
    if (parseFloat(editAllocationData.ALLOCATION) > 100) {
      alert('Allocation cannot be greater than 100%');
      return;
    }

    if (!editAllocationData.C_SOW_Start || !editAllocationData.C_SOW_End) {
      alert('Please fill in both start and end dates.');
      return;
    }

    if (editAllocationData.C_SOW_Start > editAllocationData.C_SOW_End) {
      alert('Please make sure start date is before the end date.');
      return;
    }
    ...
    try {
      const payload = {
        ...editAllocationData,
        C_SOW_Start: editAllocationData.C_SOW_Start.replace(/-/g, ''),
        C_SOW_End: editAllocationData.C_SOW_End.replace(/-/g, ''),
        IsLegacy: "0" // Assuming you want to keep the IsLegacy flag
      };
      const response = await fetchWithRetry(`${API_BASE_URL}/update/Fact_Employee_Allocation/${editAllocationData.AllocationID}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  return (
    <div>
      <Typography variant="h6" gutterBottom>Edit Allocation</Typography>
      <FormControl fullWidth margin="normal">
        ...
        <Select
          ...
          value={editAllocationData.ReportingManagerID}
      ...
      <FormControl fullWidth margin="normal">
        ...
        <Select
          ...
          id="department-id"
          name="DepartmentID"
          value={editAllocationData.DepartmentID}
          onChange={handleInputChange}
          required
        >
      ...
      <FormControl fullWidth margin="normal">
        ...
        <Select
          ...
          id="job-position-id"
          name="JobPositionID"
          value={editAllocationData.JobPositionID}
          onChange={handleInputChange}
          required
        >
      ...
      <FormControl fullWidth margin="normal">
        ...
        <Select
          ...
          id="location-id"
          name="LocationID"
          value={editAllocationData.LocationID}
          onChange={handleInputChange}
          required
        >
      ...
      <FormControl fullWidth margin="normal">
        ...
        <Select
          ...
          value={editAllocationData.ParticipationID}
          onChange={handleInputChange}
          required
        >
      ...
      <TextField
        ...
        type="number"
        name="ALLOCATION"
        value={editAllocationData.ALLOCATION}
        onChange={handleInputChange}
      ...
      <TextField
        ...
        type="text"
        name="COMMENTS"
        value={editAllocationData.COMMENTS}
        onChange={handleInputChange}
        // margin="normal"
      />
      </FormControl>
      ...
      <TextField
        ...
        type="date"
        name="C_SOW_Start"
        value={editAllocationData.C_SOW_Start}
        onChange={handleInputChange}
        // margin="normal"
        required
      ...
      <TextField
        ...
        type="date"
        name="C_SOW_End"
        value={editAllocationData.C_SOW_End}
        onChange={handleInputChange}
        // margin="normal"
        required
        labelProps={{ shrink: true }}
      ...
      <div className="mt-4">
        <button type='button' onClick={handleSaveEditAllocation} className="text-white-600 bg-blue-600 font-medium py-2 px-4 mr-2">
          Save Changes
        </button>
